...
CREATE TABLE `__new_activity` (
	...
	CONSTRAINT "activity_type_check" CHECK(type IN ('org.created', 'task.created', 'task.updated', 'task.status_changed', 'task.assigned', 'task.dependency_added', 'comment.added', 'project.created', 'member.added', 'member.role_changed', 'token.created', 'token.revoked', 'heartbeat.session', 'webhook.commit', 'webhook.received', 'meeting.applied', 'unlisted.logged')),
...
-- Appended by hand after generation (LAI-044).
--
-- SQLite drops a table's triggers with the table, and drizzle-kit implements an
-- `activity` change as DROP + rename. So the append-only guarantee installed in
-- 0000 — the one SPEC §4.8 calls "no updates, no deletes, ever" — is destroyed by
-- the rebuild above and has to be recreated here.
--
-- LAI-003's tests caught this immediately, which is the only reason it is not a
-- silent regression: the constraint would have been gone with every test still
-- green if `activity.test.ts` had only covered the happy path.
--
-- Any future migration that rewrites `activity` must do this again.
