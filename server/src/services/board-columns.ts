...
import { COLUMN_NAME_MAX } from '../db/board-columns.ts';
...
/**
 * The seeds and the function that writes them live in `db/board-columns.ts`, so
 * the migration backfill can reach them without `db/` importing `services/`
 * (CONVENTIONS §2). Re-exported here because this is where callers look.
 */
export {
  BACKFILL_COLUMNS,
  COLUMN_NAME_MAX,
