...
**Measured on a seeded instance** (LAI-244): the board gained a 146px horizontal
...
LAI-244 added `position: relative` to `.board-main`, which makes that scroller
...
- [ ] LAI-244's `position: relative` on `.board-main` is **reviewed, not
...
**Filed as LAI-244 and renumbered to 245 within minutes, by me.** CHIEF filed
their own LAI-243 (D-057's client mirror) while I was filing mine, so mine
renumbered 243 → 244 — and I then filed *this* one as 244 too, from a "lowest
free" check taken before that renumber landed. Two of my own tasks wearing one
id.

**Nothing referenced it yet**, which is the only reason renumbering was cheap and
the reason it moved rather than the in-flight one. §3's *"never renumber an
existing task"* is about ids other work points at; this had exactly one pointer,
its own filing commit.

The real lesson is D-017's: **"lowest unused number" is not a lock**, and it is
not one against *yourself* either when two filings straddle a rename.
