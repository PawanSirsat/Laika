...
- [~] ~~**Your branch is green alone.**~~ **Withdrawn — this criterion was
      wrong.** LAI-213 lives in `server/web/test/` and reads `server/src`, so
      both halves of a `*View` change are red until both land (**D-045**).
...
- [x] Full gate green — **as of the landing**: 1434 server + 560 web, format clean.
...
**Accepted**, landed with LAI-099, LAI-126 and LAI-121 in one push. 1434 server
+ 560 web, format clean.

**AC2 was wrong and is withdrawn, not ticked.** I wrote *"your branch is green
alone — the client type is its own declaration"*. It is not: LAI-213 lives in
`server/web/test/` and reads `server/src`, so both halves of a `*View` change are
red until both land. CORE found it on LAI-126 and predicted your exact two
failures **while refusing to assert them unmeasured**; you measured them. **D-045.**

**Stopping was the judgement call and you got it right.** No `clientOmits` entry,
no exemption, no disabled check — I would have sent this back for any of the
three. A drift check refusing a half-landed contract is the check working.

### Three things beyond the criteria

**`test/` held two more of the excluded kind** — an npm dependency and a React
hook's array — which my count missed because it was scoped to `src/`.
**Asserting that all three still contain `dependencies` and contain no
`blocked_by`** is better than leaving them alone: a later broad substitution
cannot quietly take them. I did not ask for that.

**AC4 caught something a compile cannot.** `blocked_by` feeds `blockedState` and
`blockers`, so a card that silently stopped rendering "blocked by" typechecks
perfectly. A browser test that a blocked card renders `.card-blocked` **naming
its blocker**, in both themes, mutation-proven — that is the first time the
LAI-227 harness has paid for itself on a real change, and it is the whole
argument for it made concretely.

**Rewriting the prose rather than substituting it.** *"The blocked_by that are
not done yet"* is what a blunt rename leaves behind, and it survives for years.
