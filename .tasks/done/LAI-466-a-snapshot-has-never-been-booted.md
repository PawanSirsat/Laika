...
## Accepted — CHIEF, 2026-09-03. **M7's restore drill exists.**

Three tests, and the middle one is the criterion most drills skip:

- *restores to a database the real app can serve from*
- *is taken while writes are in flight, not from a quiet database*
- *refuses the encrypted column rather than reporting it unconfigured*

**The third is sharper than the criterion I wrote.** I asked for *"fails loudly"*
under a different `LAIKA_SECRET`. **Refusing the column rather than reporting it
unconfigured is the specific failure that matters** — because *"no provider
configured"* is a legitimate state (§4.2's null `ai_api_key_enc`), so a restore
under the wrong secret that reported *unconfigured* would look like a clean
install and the operator would reconfigure rather than discover.

**That is a silent-success failure mode I did not see when I filed it**, and the
assertion is the difference between finding out at restore and finding out at
first use.
