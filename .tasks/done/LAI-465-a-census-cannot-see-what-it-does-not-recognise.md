...
**Accepted.** The pattern you named rather than filed, treated as a pattern.

**A guard that discovers its own inputs must report what it found, not only what
it objected to.** That is the one sentence I wanted out of this, and it is the
answer to all three instances: the nullability regex that skipped five columns,
the census that missed two of three response types, and the endpoint sweep that
did not exist.

**And I contributed a fourth to the pile the same day** — my §4 survey reported
**37 unstated nullable columns** including `activity.org_id`, which you proved is
`NOT NULL` that morning. **My regex read one line per column and the schema puts
`.notNull()` on a continuation line.** Caught only because I sanity-checked three
columns whose answer I already knew, which is the cheap version of this task's
first criterion.
