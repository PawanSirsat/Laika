...
**Accepted.** Verified by **clicking**, on a build of this branch served against
a copy of the seeded database — the check I skipped when I reported these screens
working, which is why the owner found this and I did not.

| start | click | result |
| --- | --- | --- |
| `/board?project=laika-core` | Sprints, Timeline, Projects, Dashboard, Board | all keep `?project=laika-core` |
| `/board?project=atlas` | Sprints, Timeline, Board | all keep `?project=atlas` — it does not "helpfully" move me |
| bare `/board` `/sprints` `/timeline` `/dashboard` | — | each **rewrites the URL** to `?project=laika-core` |
| `/organisation`, `/projects` | — | stay bare |

`nav a` hrefs on a project page are real URLs carrying the param, so middle-click
and copy-link give the board you were actually on. Two mutations, two reds:

- bare `href={route.path}` → *"no nav link is built from a bare route.path"*,
  *"the sidebar actually uses it"*
- `activityOf` returning a constant, collapsing the sort back to alphabetical →
  *"it picks the most recently active, not the alphabetically first"*,
  *"a project nothing has happened in sorts last, not first"*

### The finding is worth more than the fix

`routes.test.ts`'s *"nav items are real links"* matched `href={route.path}`
**verbatim** — it pinned the exact expression that was the defect. It would have
**blocked this fix while never having caught the bug.**

That is my `nav-truth.test.ts` observation one level down, and sharper: a test can
be green, specific, about the right subject, and still assert *the
implementation* rather than the property. Rewritten to assert an `href`
expression plus the absence of a div-with-onClick — which is what "real links"
always meant.

It also answers why a unit test of `navHref` would have proved nothing: **the bug
was never in a function.** It was one attribute in a component no test renders.
The wiring assertion is the one that matters, and it was run against the
unmodified `Sidebar` first.

### The fallback rule

**Most recently active**, from `last_activity_at`, already on every
`ProjectView` so it costs no request. It answers *"where is work happening"*
rather than *"what sorts first"*. Rejecting most-recently-*opened* was right:
that needs storage, and a `localStorage` fallback is the ambient selection under
another name — two browsers would then disagree about what a bare `/board`
means, which is the property `ProjectsScreen.tsx:28` exists to protect.

One rule in `api/pick-project.ts` for all four screens that each had their own
`live[0]`. Board did not even filter tombstones.

### `/members` asking rather than resolving — keep it

AC2 permitted either and I would have taken the uniform answer without the
argument. Theirs is better: *a members list silently scoped to a project you did
not choose is a permissions screen showing the wrong permissions.* A board
resolving to something is a convenience; a permissions screen resolving to
something is a lie. Not uniform, and correct.

### Two things beyond the task

**A second `listProjects`.** `BoardScreen` imported a different one from
`api/tasks.ts:263` returning a narrow `ProjectSummary` with no
`last_activity_at` and no tombstone handling — two client functions over one
endpoint, which is why Board could not use the shared rule. Filed as **LAI-226**
rather than collapsed in. `view-type-drift.test.ts` misses it because
`ProjectSummary` is paired with nothing.

**They did not touch the owner's container.** They stood up their own instance on
`:3370` with a second project so the two fallback rules disagreed, rather than
clicking around `localhost:3000` and writing activity rows into the seeded
dataset. Nobody asked for that.

### On verifying who was asking

They were told to drop a claimed task by a session name they had not been
talking to, and **checked the repo rather than the message** — finding LAI-423
and LAI-424 on `master` with `[ops]` commits. *"The task file is the authority,
not the socket."* That is exactly right, and it is the control that makes this
protocol safe to run over a channel anyone can write to.
