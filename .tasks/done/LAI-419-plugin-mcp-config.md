...
**Accepted.** Root gate `EXIT 0` — 1836 server, 644 web, **56** cli.

**`.mcp.json` unchanged, verified live rather than rewritten.** AC1 said not to
rewrite a correct file to feel like the criterion was met, and **not rewriting it
is the harder half of that instruction to follow.**

### AC4 was wrong for the third time about the same number

**It is eleven.** §7.1's first column, `registerTool` calls, and a live
`tools/list` all agree — and **`CLAUDE.md` §2 had it wrong too**, saying *"ten
listed and eleven served"* **in the passage that cites this as an example of
exactly this failure.** It was true when written and stopped being true when
LAI-433 added `laika_whoami` to §7.1. **Corrected.**

**And you found why it keeps happening**, which is worth more than the fix:

> *"A plain grep of §7.1 for backticked identifiers returns **22**, because the
> later columns name fields — `blocked_by`, `context_md`, `token_scope`. Take the
> first column and you get 11. **Every wrong number here was a grep that counted
> the wrong cells.**"*

**So the test asserts names from both sides and requires them to match, never a
number** — *"a count in prose drifts silently, a name that disappears fails"* —
**and checks both parsers found something, because two empty lists compare
equal.** That last clause is the fixture rule applied to a comparison of two
derived sets.

### AC3's escape hatch, taken explicitly and for the right reason

A real session with neither variable set: **exit 0, nothing printed at all**,
even under `--debug`.

> *"The only lever for an unprompted message is a `SessionStart` hook, and it
> fires in **every repository somebody opens**. That is the defect LAI-418's own
> criteria name for the heartbeat — **this repo decided the question once
> already, in this direction**, and I am not going to decide it the other way for
> the same plugin."*

**And nothing to file, argued rather than asserted**: *"there is no missing hook
to ask for, there is a lever whose cost this repo has already rejected."* The
criterion offered *"say so and file it"*; **declining to file, with the reason,
is the better answer to it** — a task nobody can act on is a worse record than a
README sentence somebody reads.

### The README said shipped work was unbuilt

*"What is not built yet"* listed **MCP tools**, **heartbeat hooks** and **`npx
laika init`** — all three shipped. **A document contradicting AC4 on the page
somebody opens to check whether the tools exist.** Fixed, with LAI-420 and
LAI-421 named for what genuinely is not built.
