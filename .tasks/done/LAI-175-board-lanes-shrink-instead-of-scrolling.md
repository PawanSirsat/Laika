...
## Accepted — CHIEF, 2026-09-03. **The owner's board is fixed and live.**

All three gates green. **Six widths asserted — 1920, 1600, 1440, 1280, 1100,
900** — with the last lane's right edge measured against the rail's left edge.

### Two things the task did not know, and your own test found one

**There were two `max-width: 1100px` blocks.** CORE's diagnosis quoted the first;
a second sat 370 lines lower setting `minmax(12rem, …)`. *"My own test found it —
the floor held at every width except 1100."* **That is AC1's *measure* earning its
place**, and it is why the assertion runs at six widths rather than two.

**And the old comment blamed the wrong thing.** `13rem` is 208px — **the design's
own number.** The bug was never the floor; it was **a floor with nowhere to
overflow to.** You measured the prototype in a browser rather than doing canvas
arithmetic: `206px × 5`, gap 11px, so `12.875rem`.

> *"`overflow-x: auto` here is what was missing, **and it is what carries the
> guard**."*

**A comment that misattributes a bug is worse than no comment**, because the next
person avoids the innocent thing. This is the second one today after
`services/projects.ts`, and both were found by opening the artefact.

### The green mutation, and you measured instead of re-aiming

**Removing `min-width: 0` from `.board-main > .kanban` — the exact shape of the
original collision — turned nothing red.**

> *"`min-width` computes to **`0px` either way** … because a flex item that is a
> scroll container resolves `min-width: auto` to zero. **So the mutation is a
> no-op and the green is correct.**"*

**And then you corrected your own docblock**, which had credited `min-width: 0`
with a property it does not provide. **Fourth green mutation you have reported
rather than quietly re-aimed**, and the third today that found a false comment
rather than a weak test.

### For the owner

The table is in the log. **The one thing they should eyeball is yours to have
flagged**: where the lanes do not fit, the rightmost visible lane is clipped by
the scrollport — *"the standard 'there is more' affordance and what they asked
for, but it **resembles** their original screenshot."*

**Different mechanism: before, the lane was too narrow; now it is scrolled and the
cards keep full width.** Saying that plainly, rather than hoping they do not
notice the resemblance, is the right way to hand back a fix to the person who
reported it.
