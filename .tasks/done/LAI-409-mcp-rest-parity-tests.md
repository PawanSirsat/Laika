...
**Accepted. M3's server chain is complete** — 402 → 403 → 406 → 407/408 → 409.

Three mutations, three reds:

| mutation | result |
| --- | --- |
| register a tool with neither twin nor exemption | *"these MCP tools have neither a REST twin here nor a named exemption: `probe_tool`"* |
| exempt `add_comment`, which **does** have a twin | *"exempts exactly one tool, with a reason that cites the decision"* — the entry was rejected |
| `start_working` reaching for `changeStatus` instead of `claimTask` | 4 red, each naming the tool |

**The derived list is the load-bearing half and it works.** `PAIRS` and `EXEMPT`
are checked against a live `listTools()`, so a tool added in six months appears
in neither map and the completeness test names it. An exemption protects the one
case somebody thought about; this protects the case nobody did.

**`finish_task`'s twin as a composite was the right call.** There is no
`POST /tasks/:id/finish` because a human finishing work does two things — moves
the task to review and says what they did — so its twin is those two REST calls
in sequence. Exempting it would have left **the tool most likely to drift as the
one nothing checked.**

### The measurement that explains what this test is for

Their first attempt at AC6 mutated the **shared service** — `createTask` writing
`task.updated` — and the `create_task` pair **stayed green**, because both halves
call the same service and it moved them together.

> **Parity cannot be broken by changing the thing both halves share.**

That is the property arriving as a measurement rather than an argument. The
layering rules already guarantee routes and tools reach data only through
`services/`; this test confirms that property rather than propping it up, and the
only mutation that can break it is a **tool diverging from its route** — which is
exactly what the three that worked all did.

### Two of three mutations were wrong before one was right

Reported unprompted. The first went red on a *different* test — *"I would have
recorded AC6 satisfied had I read only the exit code"* — and the second "made a
read tool write" by adding a call that reads. **The test was fine both times.**

That is the fourth instance this week of a probe being wrong before the code was,
and the second where the wrong probe would have been recorded as a pass. Reading
*which* test went red, not merely that something did, is the check.
