...
**Accepted.** Two mutations, two reds, each landed-checked before running.

| mutation | result |
| --- | --- |
| `system` collapses to a plain person | red: *"a system row is the system, not an anonymous person"*, *"all three kinds are distinguishable"* |
| `agent` and `system` share a badge value | red: *"no two kinds produce the same badge"* |

**That second test is a property, not a contingent fact** — *"no two kinds
produce the same badge"* stays true whatever a fourth `actor_kind` would be.
D-037 applied without being asked to.

**AC2 and AC3 verified in the diff, not taken on report.** `--pur` and `--tx3`
only, both already assigned in `docs/design/README.md`; no new colour anywhere.
The rail's shapes differ by `--radius-sm` against `--radius-pill` — a real
geometric difference, not two hues.

**The `"someone edited this task"` fix is the right one and for the right
reason.** `activity_system_actor_check` — `(actor_id IS NULL) = (actor_kind =
'system')` — *proves* a null actor on an activity row is not a person, so the old
rendering was a wrong label rather than a missing one. Leaving `personName`
untouched for `created_by` and `comment.author_id`, where a null genuinely is a
departed user, is the distinction that makes this a new function rather than an
edit. And `describeActor` being the single place both sites ask is what stops
them drifting again — which is what had happened.

**Leaving the dashboard alone was right.** It already had all three. Churning
correct code to touch every site named in a criterion is not what the criterion
asked for.

### The stale dev server is the finding worth keeping

`/api/v1/tokens` 404'd while `app.ts:185` plainly mounted it — the server had
been serving **pre-merge code for hours**, started with `tsx` rather than
`tsx watch`. Any *"that endpoint does not exist"* concluded in that window would
have been false.

**That is a measured absence produced by a stale process**, which is the same
family as §5's setup-step rule: the step assumed to have worked is the one that
did not. Restarting after a merge belongs in the same habit as reading
`git show --stat` and asserting a fixture's status.

### One behaviour change, accepted and filed

An actor absent from the members map used to render a raw ULID and now renders
`"Someone"`. Accepted — a ULID is not a name and §5.1 is clear about ids where a
human reads.

**But there is a case the code comments do not anticipate**, and I checked rather
than assumed: `members` comes from the **project** members endpoint, and org
Owners and Admins hold implicit `lead` **without a membership row**. So an Admin
acting on a project they are not a member of is not in that map — and that is not
the departed-user case, nor transient. It is the ordinary way an Admin works.

Before this task they rendered as a ULID: also wrong, but *visibly* wrong.
`"Someone"` is worse in exactly one way — **it looks deliberate.** Filed as
**LAI-416**, not folded in: the criteria never mentioned unresolved actors, and
unifying the two sites is what surfaced it.

### On the web suite

*"The web suite does not typecheck components"* is true of the **suite** — but
`server/web/tsconfig.json` includes `src/**/*.tsx`, so `pnpm typecheck` does
cover them and the gate would have caught the missing import. Worth being precise
about: the hole is **behavioural coverage** of `.tsx`, not type coverage.
