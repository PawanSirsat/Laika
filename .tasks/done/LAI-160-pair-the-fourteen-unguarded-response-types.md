...
**Accepted. `PAIRS` 7 → 20.** Root gate `EXIT 0` — 1758 server, 604 web, 49 cli.

**Twelve of the fourteen agreed already**, which confirms LAI-444 from the other
side: the convention was being followed and the guard simply was not reading it.
**M5's exit is not delayed.**

### `SetupStatusBody.system` — declared, not exempted

> *"The omission was never a decision — LAI-158 was already filed to render that
> panel. **An exemption would have recorded an oversight as a choice.**"*

**That is exactly the distinction CORE put in the Notes, applied on its first
real case.** `clientOmits` is for a field the client deliberately does not
mirror; this one it had simply never had. Following `Task.blocks` and
`Task.sprint_id` — declared, with the task that renders it named — gives LAI-158
a type to render from rather than a gap to discover.

### `ProjectView` — a limitation of the new census, found and not routed around

**It was never unpaired.** `PAIRS` has `ProjectSummary → Project`,
`ProjectSummary extends ProjectView`, and **`fieldsOf` resolves `extends`** — so
every `ProjectView` field was already compared. Adding the entry the table asked
for made the check **red correctly**, by asserting the base sends the five fields
the summary derives.

> **"LAI-444's census counts a literal name in `PAIRS` and cannot see coverage
> inherited through `extends`. Its total is not a to-do list."**

**And you tried to reclassify it and turned `names a client type that exists`
red** — because the map's second column *is* the client type and prose fails it.
**There is no slot for "covered another way."**

**Leaving the row untouched and explaining why in CORE's file was the right
call.** *"That is the finding, not an obstacle to route around, and designing one
is CORE's call."* Filed as **LAI-445** so the census stops overstating by one.

### Checking the pairs are not vacuous before trusting the green

> *"A pair whose parser finds nothing compares an empty set to an empty set and
> passes. **Thirteen new entries would have been thirteen new places for that.**"*

**All twenty probed for field counts first** — `TokenView` 9 vs 9, `InviteView`
9 vs 9, `SetupStatusBody` 2 vs 2, every one non-zero — then two mutations, both
red. **Nobody asked for that**, and it is the difference between adding thirteen
guards and adding thirteen things that look like guards.

**`TokenView` is compared for the first time**: identical name on both sides,
nine fields each, and until today nothing looked at either.
