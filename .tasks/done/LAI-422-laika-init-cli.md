...
**Accepted. M4's exit criterion runs.** 1632 server, 585 web, **19 cli** where
the gate printed `# pass 0` this morning. Reviewed **whole**, not only the
end-to-end, because the earlier half reached `master` in a merge of mine before
anyone had looked at it.

### Three defects, none visible to a unit test

**Each question closed stdin for the next.** *"Every call was correct alone; the
sequence failed."* That is the clearest statement of why an end-to-end criterion
was worth writing: a suite of correct units composing into a program that exits
silently after printing `Email:`.

**The password was echoed in full.** A **paused** `readline` still owns the
terminal's mode, so `setRawMode(true)` never took. **AC2 was the criterion most
obviously met on paper and it was not met**, and only a pty could show it —
because a pipe has nothing to echo.

**`laika init < answers` could not work at all.** A piped `readline` closes at
EOF, which arrives while `init` is awaiting a network call. Fixing it rather than
working around it is what let the suite drive the CLI as a subprocess, which is
the reason it was the right call and not merely the thorough one.

**And the fourth, in your own test:** `execFileSync` blocking the event loop so
the in-process stub board could never answer the child — *"a timeout with no
failing assertion; a hung test that reads as a hung CLI."*

### The gap I measured, and it is not yours — LAI-441

**Nothing catches the echo coming back.** Replacing `closePrompt()` with
`shared?.pause()` leaves the suite at **19 passing, 0 failing**. One word from
undone, and the gate silent.

**Your test file says so itself** — *"the closest the suite can get to the pty run
recorded on the task without adding a dependency"* — which is the comment rule
working: it claims exactly what it covers and no more. And LAI-422's Notes
**forbid a new dependency**, so you were not permitted to close it.

**Filed as LAI-441**, which names the dependency so one may be added, and offers
the shape that needs none: assert the *mechanism* — the shared interface is
**closed, not paused**, before `setRawMode(true)` — with a fake stdin recording
call order. Weaker, ships today, and fails on exactly the edit that caused the
bug.

### The verification I would have accepted a weaker version of

**Idempotence measured on the board** — tokens before the second run 3, after 3,
stored token unchanged — rather than inferred from the code path.

**And AC6's fourth failure named as stub-only.** *No permission to mint* is
unreachable live, because a viewer's token is **forced** to `read_only` rather
than refused (LAI-410), so no active account can produce that `403`. Saying
*"partly verified, and here is which part"* is worth more than four ticks, and it
is recorded in LAI-441's Notes as honestly unreachable rather than merely
uncovered.

**Using your own instance on `:3370` rather than the owner's board** was right —
no token minted on a board somebody else is looking at.
