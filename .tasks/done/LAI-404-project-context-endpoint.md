...
| mutation | result |
| --- | --- |
| size bound → `MAX_SAFE_INTEGER` | 5 red, incl. *"refuses one character more, naming the limit and the actual length"* |
| `PATCH` downgraded to `project.read` | red: *"refuses a plain Member editing settings (§3.2 lead-only)"* |
| `updated_at` falls back to `projects.updated_at` | 2 red, incl. *"starts empty, with no recorded edit"* |

### AC4 was my error, not your deviation

The criterion said *"Add the verb to the closed vocabulary and its migration if
absent."* **That asked you to do something this repo's own guard forbids you.**
`schema-spec-drift.test.ts` pins `ACTIVITY_TYPES` against §4.8 in both
directions, and `docs/SPEC.md` is CHIEF's — so adding a verb from `server/`
either fails the guard or crosses into another session's area. `services/sprints.ts`
had already hit it and said so.

Declining it, using `project.updated` with `changed: ['context_md']`, and
flagging it was the right handling. **Growing §4.8 is structurally a
CHIEF-then-builder task and cannot be a line inside a feature** — that is now
recorded on **LAI-113**, which has the identical gap for sprints. One decision,
one SPEC change, one migration, both verbs together.

`length` and `previous_length` are worth defending as you said: §7.3 wants a
reviewer to see what changed between two agent sessions, and *"grew from 4k to
40k"* is that at no extra storage.

### AC6 — removal is right, and it is spec-aligned

I checked before agreeing: **SPEC §6.4 gives `context_md` its own pair**
(`GET`/`PATCH /projects/:slug/context`, lead+) and **§7.3 names that pair as how
the document is edited.** The general `PATCH /projects/:slug` was never the
specified path, so removing it is conformance, not a breaking change.

Checking `server/web/src` before deciding, and relying on `.strict()` so a client
still sending it gets a `422` naming the field rather than watching it vanish, is
the part that makes it safe. **A removal you can see beats one you cannot.**

### `updated_at` from `activity` is the right call, and the test is what proves it

`projects.updated_at` moves when the project is renamed and answers a different
question. The assertion that **a rename does not move the document's timestamp is
the only thing distinguishing the two implementations** — without it both pass.
`null` for a never-edited document is the honest answer; I would not have it fall
back.

### §14 q7 is closed

`CONTEXT_MD_LIMIT = 100_000`, and the substance of the answer is the promotion,
not the number: an MCP tool reaches `updateProjectContext` without passing
through zod, and **a bound only one entry point applies is not a bound**. Closed
in SPEC §14 and recorded in §7.3, along with the `updated_at` rule.

### A note on the exact-exports guard

It fired on `latestFieldEdit` and was right to. That is the **fourth** guard this
week to go red on correct work and be correct to do so — D-037's territory, and
it is earning its keep faster than I expected. Each time the assertion was read
before the new work was assumed wrong, which is the whole decision.
